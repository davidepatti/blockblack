...
def run_recipe(manifest_path: Path, trivia: str, out_dir: Path) -> None:
    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    key = trivia.strip()
    if key not in manifest["trivia"]:
        raise DecodeError(f"trivia {trivia!r} is not present in {manifest_path}")
    recipe = manifest["trivia"][key]
    ...
    print(f"Trivia: {recipe.get('title', '')}")
...
def main() -> int:
    ...
    try:
        run_recipe(args.manifest, args.trivia, args.out)
